...
@interface DNCollectionViewController ()
...
-(void)getDataWithType:(int)type
{
    ...
    DLHttpsBusinesRequest *request= [DLHttpRequestFactory getCollectionListWithNumber:@"20" offset:offsetString];
    ...
    request.requestSuccess = ^(id response)
    {
        ...
        DLJSONArray * jsonArray = [dataObject getJSONArray:@"favorite"];
...
-(void)noDataMethods
{
    if (self.dataArray.count==0) {
        ...
        [self showNoDataView:self.collectionView noDataString:@"没有收藏记录"
            ...
            IndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell.selected) {
        ...
        [self.videoIdArray addObject:[NSString stringWithFormat:@"%ld",(long)videoModel.favoriteid]];
        
    }else
    {
        
        [self.videoIdArray removeObject:[NSString stringWithFormat:@"%ld",(long)videoModel.favoriteid]];
