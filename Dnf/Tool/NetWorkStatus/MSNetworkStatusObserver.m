...
@property (nonatomic,strong) MSReachability *reachability;
...
#pragma mark - 初始化
static MSNetworkStatusObserver* MSGlobalCoreStatusInstance = nil;
...
+(instancetype)shareInstance
{
    ...
    dispatch_once(&NetworkStatus, ^{
        ...
        MSGlobalCoreStatusInstance = [[MSNetworkStatusObserver alloc]init];
...
+(void)beginNotiNetwork:(id<MSCoreStatusProtocol>)listener
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:status
                                             selector:@selector(coreNetWorkStatusChanged:)
                                                 name:MSReachabilityChangedNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:status
                                             selector:@selector(coreNetWorkStatusChanged:)
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.reachability = [MSReachability reachabilityForInternetConnection];
