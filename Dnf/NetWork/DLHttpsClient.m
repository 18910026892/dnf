...
static NSDictionary *sg_httpHeaders       = nil; //请求头

static NSMutableArray *sg_requestTasks;
static BOOL sg_cacheGet = YES;
static BOOL sg_cachePost = NO;

static DLRequestType  mRequestType  = DLRequestTypeJSON; // 请求数据格式 缺省为JSON
static DLResponseType mResponseType = DLResponseTypeJSON; // 返回数据格式 缺省为JSON
static BOOL mShouldAutoEncode = NO;                      // 是否自动编码请求地址

static DLNetworkStatus sg_networkStatus = DLNetworkStatusReachableViaWiFi;

static BOOL sg_shouldCallbackOnCancelRequest = YES;  // 是否要取消回调 缺省为YES
...
static inline NSString *cachePath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/DLNetworkingCaches"];
}
...
+ (void)cancelAllRequest {
    @synchronized(self) {
        [[self allTasks] enumerateObjectsUsingBlock:^(DLURLSessionTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([task isKindOfClass:[DLURLSessionTask class]]) {
                [task cancel];
            }
        }];
...
+ (DLURLSessionTask *)_requestWithUrl:(NSString *)url
                          refreshCache:(BOOL)refreshCache
                             httpMedth:(NSUInteger)httpMethod
                                params:(NSDictionary *)params
                              progress:(DLDownloadProgress)progress
                               success:(DLResponseSuccess)success
                                  fail:(DLResponseFail)fail {
    ...
    DLURLSessionTask *session = nil;
    
    if (httpMethod == 1) {
        if (sg_cacheGet) {
            if (sg_shoulObtainLocalWhenUnconnected) {
                if (sg_networkStatus == DLNetworkStatusNotReachable ||  sg_networkStatus == DLNetworkStatusUnknown ) {
                    id response = [DLHttpsClient cahceResponseWithURL:absolute
                                                           parameters:params];
                    if (response) {
                        if (success) {
...
+ (DLURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    AFHTTPSessionManager *manager = [self manager];
    DLURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSData *imageData = [NSData dataWithContentsOfFile:uploadingFile];
...
//+ (DLURLSessionTask *)uploadFileWithPath_wzx:(UIImage *)path
//                                         url:(NSString *)url
...
+ (AFHTTPSessionManager *)manager {
    @synchronized (self) {
        ...
        if (sg_sharedManager == nil || sg_isBaseURLChanged) {
            ...
            switch (mRequestType) {
                case DLRequestTypeJSON: {
                    manager.requestSerializer = [AFJSONRequestSerializer serializer];
                    break;
                }
                case DLRequestTypePlainText: {
            ...
            switch (mResponseType) {
                case DLResponseTypeJSON: {
                    manager.responseSerializer = [AFJSONResponseSerializer serializer];
                    break;
                }
                case DLResponseTypeXML: {
                    manager.responseSerializer = [AFXMLParserResponseSerializer serializer];
                    break;
                }
                case DLResponseTypeData: {
...
+ (void)detectNetwork {
    ...
    [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == AFNetworkReachabilityStatusNotReachable){
            sg_networkStatus = DLNetworkStatusNotReachable;
        } else if (status == AFNetworkReachabilityStatusUnknown){
            sg_networkStatus = DLNetworkStatusUnknown;
        } else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
            sg_networkStatus = DLNetworkStatusReachableViaWWAN;
        } else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
            sg_networkStatus = DLNetworkStatusReachableViaWiFi;
        }
    }];
}
...
+ (void)successResponse:(id)responseData callback:(DLResponseSuccess)success {
    if (success) {
...
+ (void)handleCallbackWithError:(NSError *)error fail:(DLResponseFail)fail {
    if ([error code] == NSURLErrorCancelled) {
