...
@implementation DNVideoTableViewCell
...
-(void)setVideoModel:(DNVideoModel *)videoModel
{
    ...
    self.watchLabel.text = [NSString stringWithFormat:@"%ld次观看",videoModel.watches];
...
-(UIImageView*)coverImageView
{
    if(!_coverImageView)
    {
        _coverImageView = [[UIImageView alloc]initWithFrame:CGRectMake(15, 0, KScreenWidth-30, 180)];
        ...
        [_coverImageView addSubview:self.watchLabel];
...
{
    if (!_watchLabel) {
        _watchLabel = [[UILabel alloc]initWithFrame:CGRectMake(9, 180-25, KScreenWidth/2-15-9, 17)];
...
{
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, 180+7, KScreenWidth-30,21)];
